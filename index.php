			...
			<div class="row">
				<div class="col-md-9 no-padding">
					<div class="wrap--outer">
						...
						<p class="main-heading">Search for Lab List</p>


						<!-- Search Form -->
						<form accept-charset="utf-8" id="tfnewsearch" method="get" action="http://localhost/workstation/lablist.php">
							<div class="wrap--inner">
								<div class="wrap">
									<select name='division' size='20' class='list filterSelect' data-target='select-family' id='select-city'>
										...
										while($val=mysqli_fetch_assoc($res)){
											?>
											<option style="cursor:pointer"  class="pointOption" value="<?php echo $val['division']?>" data-reference="<?php echo $j++?>"><?php echo $val['division'].' ('.countV("division",$val['division']).')'?></option>
										<?php } ?>
									</select>
								</div>
									...
									<select name='district' size='20' class='list filterSelect' data-target='select-name' id='select-family' data-allowempty>
										...
										while($parent=mysqli_fetch_assoc($res)){
											...
											while($child=mysqli_fetch_assoc($val)){
												?>
												<option style="cursor:pointer" value="<?php echo $child['district']?>"  data-reference="<?php echo $j++; ?>" data-belongsto="<?php echo $m?>"> <?php echo $child['district'].' ('.countV("district",$child['district']).')'?></option>
												<?php
											}
											$m++; }
										?>
										...
										while($parent=mysqli_fetch_assoc($res)){
											...
											while($children=mysqli_fetch_assoc($val)){
												...
												while($child=mysqli_fetch_assoc($va)){
													?>
													<option style="cursor:pointer" value="<?php echo $child['sub_district']?>"   data-belongsto="<?php echo $m?>"> <?php echo $child['sub_district'].' ('.countSubDistrict($children['district'],$child['sub_district']).')'?></option>
													<?php
												}
												$m++; }
										}
